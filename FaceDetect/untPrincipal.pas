...
procedure TfrmPrincipal.ChangeImageExposure(src: TBitmap; k: Single);
var
  RGB: PRGBArray;
...
procedure TfrmPrincipal.LoadFaces(Sender: TObject);
begin
  DoSQLiteconnect;
  LoadDatabase;
end;

procedure TfrmPrincipal.VariantToStream(const v : olevariant; Stream : TMemoryStream);
var
  p : pointer;
begin
  Stream.Position := 0;
...
procedure TfrmPrincipal.VariantToJPEGToBMP(aValue:OleVariant; var aJPEG:TJPEGImage; var aBMP:TBitmap);
var
...
procedure TfrmPrincipal.LoadDatabase;
var
  dbFaces: pIplImage;
  xID:integer;
  xFace:OleVariant;
...
procedure TfrmPrincipal.WMQueryEndSession(var Message: TMessage);
begin
  SessionEnding := True;
  Message.Result := 1;
end;
...
function TfrmPrincipal.DoSQLiteConnect : boolean;
begin
  try
...
procedure TfrmPrincipal.InitDB;
begin
...
procedure TfrmPrincipal.InitPaths;
begin
...
procedure TfrmPrincipal.StopCapture;
begin
...
procedure TfrmPrincipal.StartCapture(camnum:integer);
begin
...
procedure TfrmPrincipal.OnIdle(Sender: TObject; var Done: Boolean);
var
  bFound : Boolean;
...
begin
  ...
  begin
    ...
    begin
      ...
      //Ref: #13392
      MyfaceImage.MyCapture := cvCreateCameraCapture(0);
      cvSetCaptureProperty(MyfaceImage.MyCapture, CV_CAP_PROP_FRAME_WIDTH, 640);
      cvSetCaptureProperty(MyfaceImage.MyCapture, CV_CAP_PROP_FRAME_HEIGHT, 480);
...
procedure TfrmPrincipal.StreamToVariant (Stream : TMemoryStream; var v : OleVariant);
var
  p : pointer;
begin
...
function TfrmPrincipal.JPEGToVariant(aJPEG : TJPEGImage) : OleVariant;
var
  Stream        : TMemoryStream;
begin
  try
...
procedure TfrmPrincipal.JustDetect();
var
i: Integer;
inputFace: pCvSeq;
...
inputPt1, inputPt2: TCvPoint;
begin
  inputFace := cvHaarDetectObjects(MyfaceImage.MyInputImage, FaceCascade, MyfaceImage.MyStorage, 1.1, 10,
...
function TfrmPrincipal.DetectAndDraw(const faceImage: pfaceImage) : Boolean;
var
  I: Integer;
  lab: Integer;
...
begin
  try
    inputFace := cvHaarDetectObjects(faceImage.MyInputImage, FaceCascade, faceImage.MyStorage,
    1.2, 10, 2, cvSize(99, 99), cvSize(0, 0));
    ...
    begin
      ...
      begin
        sAux := 'Name: ' + fxnames[lab];
        Label1.Caption := 'Nome: ' + fxnames[lab];
...
begin
  Button4.Enabled := ((Edit1.Text <> 'Digite um nome') and (Edit1.Text <> '') and (Label1.Caption = 'Nome: '));
end;
...
procedure TfrmPrincipal.ClearWindow;
begin
  MyfaceImage := nil;
